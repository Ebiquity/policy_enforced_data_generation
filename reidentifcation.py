...
def analyse_data(label):
    ...
    df_synthetic = pd.read_csv("./synthetic_table/farmer_survey_synthetic_without_privacy.csv")
    ...
    y_train = df_synthetic[label].astype('int')
    X_train = df_synthetic.drop(label, axis=1)

    gbc_result = gradientBoostingClassifier(X_train, X_test, y_train, y_test)
    print("Accuracy:" + str(gbc_result) + "\n")


analyse_data("land_slope")
analyse_data("crop_count")
